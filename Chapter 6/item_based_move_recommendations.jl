...
function movie_similarity(target_movie)
  ...
  for movie in names(movies[:, 2:end])
    ...
    common_users = ratings[Array(ratings[movie] .>= 0) .& Array(ratings[target_movie] .> 0), :]

    correlation = try
      cor(Array(common_users[movie]), Array(common_users[target_movie]))
    catch
      0.0
    end
